...
**Huge Problem(Pain Point):** 현장에서 모은 CSV가 파일별로 흩어지면 위치·작성자·시간에 따른 WiFi 품질을 비교하기 어렵다.

**솔루션 한 줄 정의:** CSV 업로드·필터·시계열·분포 분석을 하나의 웹 대시보드로 통합한다.

**현재 상태:** WiFi 시스템 대시보드

**문제 해결 중심의 사고 흐름**

1. **관찰** — 측정 파일을 개별로 열어서는 장소와 기간별 품질 저하 패턴을 빠르게 찾기 어려웠다.
2. **선택** — 원본 파일 메타데이터와 측정 행을 중앙 저장하고 동일 필터를 조회·모니터링·분석에 공유했다.
3. **구현** — React·Supabase 기반 업로드, 작성자·위치·날짜 필터, 6개 시계열 차트와 분포 히스토그램을 구현했다.
4. **검증과 한계** — Vercel 배포 URL과 v3 데이터 스키마가 문서화되어 있다. 시계열·상관·이상치의 일부 고급 분석은 준비 중이다.
